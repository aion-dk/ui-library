...
### Dark mode: an inherited attribute, not a prop

Colour mode is the second consumed contract. The library enables Bootstrap's native dark
mode (`$enable-dark-mode: true`, `$color-mode-type: data`), so the host app owns the
switch: it sets `data-bs-theme="dark"` on any ancestor and every descendant re-resolves
its `--bs-*` tokens. Components only have to paint with tokens that follow the mode —
`text-body`, `text-body-70`, `bg-body-80` and friends, generated by `_body_opacity.scss` —
and put the cases tokens can't express behind a `[data-bs-theme="dark"]` selector in SCSS.

The earlier approach was a `theme: "light" | "dark"` prop that each component threaded
into class names (`AVFoo--text-dark`). It scaled badly: every wrapper had to forward the
prop, nested components could disagree with their parent, and dark mode couldn't follow
an ancestor the library doesn't own. The attribute inverts that — one host-owned switch,
zero prop plumbing, and CSS resolves the cascade. The components consumed by conference
and electa-backoffice are migrated; `AVVerticalStep` and `AVAnimatedMenuButton` still take
the legacy prop and are migrated when next touched.

The trade-off is testability: colour mode is now pure CSS inheritance, so jsdom unit tests
can't assert it (no Bootstrap stylesheet, no cascade). Specs therefore assert the *class
contract* — which cells get body tokens versus a `--highlighted` override — and the visual
